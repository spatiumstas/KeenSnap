...
script_update() {
  ...
  if [ -f "$TMP_DIR/$SCRIPT" ]; then
    ...
    if [ "$mode" = "silent" ]; then
      logger -p notice -t KeenSnap "Скрипт обновлён в silent-режиме"
      exit 0
    else
    ...
    if [ "$mode" = "silent" ]; then
      logger -p err -t KeenSnap "Ошибка при обновлении скрипта"
      exit 1
    else
      print_message "Ошибка при обновлении скрипта" "$RED"
    fi
  fi
}
